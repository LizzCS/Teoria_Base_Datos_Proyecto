    ...
    public class Graficos
    {
            ...
            using (var conn = Conexion.GetConnection())
            {
                ...
                using var cmd = new SqlCommand(@"
                    SELECT c.id_categoria, c.nombre,
                           ISNULL(SUM(t.monto), 0), COUNT(t.id_transaccion)
                    FROM categoria c
                    INNER JOIN subcategoria s        ON s.id_categoria          = c.id_categoria
                    INNER JOIN presupuesto_detalle pd ON pd.id_subcategoria      = s.id_subcategoria
                    INNER JOIN transaccion t          ON t.id_presupuesto_detalle = pd.id
        ...
        private static async Task<string> ObtenerHtmlReporte3(int idPresupuesto, int anio, int mes)
        {
            ...
            using (var conn = Conexion.GetConnection())
            {
                conn.Open();
                using var cmd = new SqlCommand(@"
                    SELECT s.nombre, pd.monto_mensual, ISNULL(SUM(t.monto), 0)
                    FROM presupuesto_detalle pd
                    INNER JOIN subcategoria s ON s.id_subcategoria       = pd.id_subcategoria
                    LEFT  JOIN transaccion  t ON t.id_presupuesto_detalle = pd.id
                                              AND t.anio_transaccion = @anio
                                              AND t.mes_transaccion  = @mes
                    WHERE pd.id_presupuesto = @idP
                    GROUP BY s.nombre, pd.monto_mensual
                    ORDER BY s.nombre", conn);
                cmd.Parameters.AddWithValue("@anio", anio);
                cmd.Parameters.AddWithValue("@mes",  mes);
                cmd.Parameters.AddWithValue("@idP",  idPresupuesto);
                using var r = cmd.ExecuteReader();
                while (r.Read())
                {
        ...
        private static async Task<string> ObtenerHtmlReporte4(int anioDesde, int mesDesde, int anioHasta, int mesHasta)
        {
            ...
            using (var conn = Conexion.GetConnection())
            {
                conn.Open();
                using var cmd = new SqlCommand(
                    "SELECT id_categoria, nombre FROM categoria WHERE tipo_categoria = 'gasto' ORDER BY nombre", conn);
                using var r = cmd.ExecuteReader();
                while (r.Read())
                {
        ...
        private static async Task<string> ObtenerHtmlReporte5(int anio, int mes)
        {
            ...
            using (var conn = Conexion.GetConnection())
            {
                conn.Open();
                using var cmd = new SqlCommand(@"
                    SELECT o.id_obligacion, o.nombre, o.monto_fijo_mensual, o.dia_del_mes,
                           ISNULL((SELECT TOP 1 t.fecha_hora_registro
                                   FROM obligacion_transaccion ot
                                   INNER JOIN transaccion t ON t.id_transaccion = ot.id_transaccion
                                   WHERE ot.id_obligacion = o.id_obligacion
                                     AND t.anio_transaccion = @anio AND t.mes_transaccion = @mes
                                   ORDER BY t.fecha_hora_registro DESC), NULL) AS ultimo_pago
                    FROM obligacion_fija o WHERE o.esta_vigente = 1 ORDER BY o.dia_del_mes", conn);
                cmd.Parameters.AddWithValue("@anio", anio);
                cmd.Parameters.AddWithValue("@mes",  mes);
                using var r = cmd.ExecuteReader();
                while (r.Read())
        ...
        private static async Task<string> ObtenerHtmlReporte6(int idPresupuesto, int anio, int mes)
        {
            ...
            using (var conn = Conexion.GetConnection())
            {
                ...
                using var cmd = new SqlCommand(@"
                    ...
                    LEFT  JOIN transaccion  t ON t.id_presupuesto_detalle = pd.id
                                              AND t.anio_transaccion = @anio AND t.mes_transaccion = @mes
                    WHERE pd.id_presupuesto = @idP AND c.tipo_categoria = 'ahorro'
                    GROUP BY s.nombre, pd.monto_mensual ORDER BY s.nombre", conn);
                cmd.Parameters.AddWithValue("@anio", anio);
                cmd.Parameters.AddWithValue("@mes",  mes);
                cmd.Parameters.AddWithValue("@idP",  idPresupuesto);
                using var r = cmd.ExecuteReader();
